...
//
//																				if (c.moveToFirst()) {
//																								srcPath = c.getString(columnIndex);
//																				}
//																				c.close();
																srcPath = uri.getPath();
				...
				public void submit() {
								// 首先判断资料是否填写完整
								if (!flag || name_et.getText().toString().isEmpty() || address_et.getText()
																.toString()
																.isEmpty() ||
																contact_et.getText().toString().isEmpty() || service_et.getText()
																.toString()
																.isEmpty()) {
												ShowToast("请将资料填写完整");
												return;
				...
				private void completeUserInfo(final String headerAddress) {
								...
								NetworkSingleton.getInstance(getApplicationContext())
																.addToRequestQueue(complete_user_info_request);
				}
